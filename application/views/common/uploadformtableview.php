...
$( document ).ready(function() {
	...
	uploadFormJS(div,uplabel, "<?php echo fileserver_url.'proses_upload'; ?>", 1, 100,1,"<?php echo $this->security->get_csrf_hash(); ?>",tableid, data_id = null)

	getCover(div, idnyaheader, tableid)
});


function uploadFormJS(div, label, url, file_max_upload, file_max_size,user_id,csrf,tableid,data_id = null){
							...
							'<div class="col-lg-9">'+
								...
								'<div class="dropzone dropzone-queue mb-2" id="'+div+'_dropzone">'+
									...
									'<div class="dropzone-panel mb-lg-0 mb-2">'+
										...
										'<a class="dropzone-upload btn btn-sm btn-light-primary me-2" style="display:none">Unggah All</a>'+
										'<a class="dropzone-remove-all btn btn-sm btn-light-primary" style="display:none">Hapus All</a>'+
									'</div>'+
									'<!--end::Controls-->'+
										'<!--begin::Items-->'+
		...
		init: function() {
			...
			this.on("success", function(file, response) {
				...
				if(obj.message == 'success'){
					$(id +' .dropzone-items .dropzone-item:last-child .dropzone-file .dropzone-filename').append('<input type="text" style="display:none" class="form-control form-control-sm" name="evidence_detail['+tableid+']" value="' + obj.id +'" />');
...
function uploadFormJSCustom(div, div_text, label, url, file_max_upload, file_max_size,user_id,csrf,tableid,data_id = null){
...
function getCover(div, id, tableid){
						...
						for (var key in data) {
							...
							var html = '<div class="dropzone-item" id="eviden_data_'+data[key].evidence_id+'">'+
												...
												'<div class="dropzone-file">'+
													...
													'</a>'+
													'<input type="text" style="display:none" class="form-control form-control-sm" name="evidence_detail['+tableid+']" value="' + data[key].id +'" />' +
